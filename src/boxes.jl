struct Box{L<:LineStyle,U<:LineStyle,R<:LineStyle,D<:LineStyle}
    lborder
    uborder
    rborder
    bborder
...
function Box(x, y, width, height; left::Type{LineStyle{L,T1}} = LineStyle(Light, Solid),
    ...
    rborder = Line(_x, _y, _l, Vertical; style = right, pstyle = pstyle)
    _x, _y, _l = x + 1, y + height - 1, width - 2
    bborder = Line(_x, _y, _l, Horizontal; style = bottom, pstyle = pstyle)
    corners = [
